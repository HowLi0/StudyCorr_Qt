...
namespace StudyCorr
{
	...
	{
		if (!interp_coefficient.empty())
		{
			interp_coefficient.clear();
		}
...
#pragma omp parallel for
	...
	float BicubicBspline::compute(Point2D& location)
	{
		...
		float value = 0.f;
		Array2D& coefficient = interp_coefficient[y_integral][x_integral];
	...
	void TricubicBspline::prepare()
	{
		...
		Array3D conv_buffer = new3D(dim_z, dim_y, dim_x);
